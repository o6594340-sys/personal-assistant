...
def ai_parse_voice(text: str, today: str) -> dict:
    prompt = (
        f"Сегодня {today}. Пользователь сказал: «{text}». "
        "Определи, это задача или идея. "
        "Если задача — извлеки название, тип (work/personal/rest/growth), дату (если упомянута) и время. "
        "Если идея — извлеки суть и дату напоминания (если упомянута). "
